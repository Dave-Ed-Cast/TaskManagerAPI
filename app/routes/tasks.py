...
async def create_task(task: models.TaskCreate, user=Depends(get_current_user)):
    user_id = user["id"]
    ...
    task_id = await crud.create_task(task.title, task.description, user_id, is_shared_task)
...
async def list_tasks_for_user(current_user: dict = Depends(get_current_user)):
    return await crud.get_tasks_for_user(current_user)


# ===== DELETE methods =====
@router.delete("/{task_id}", response_model=dict)
async def delete_task(task_id: int, user=Depends(get_current_user)):
    success = await crud.delete_task(task_id, user["id"], user["is_admin"])
    if not success:
